...
async def start_next_task(bot, student_id):
    ...
    try:
        ...
        try: await bot.pin_chat_message(chat_id=int(student["telegram_id"]), message_id=msg.message_id)
        except: pass
    except Exception as e:
        logger.error(f"Error sending start task message: {e}")
    return True
...
async def handle_mentorship_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("m_done_"):
        ...
        if new_end.tzinfo is None: new_end = IST.localize(new_end)
        ...
        try:
            ...
            for student in students:
                ...
                for task in manual_tasks:
                    if task.get("status") != "pending" or task.get("followup_sent"):
                        continue
                        
                    start_at = task.get("scheduled_at")
                    allotted = task.get("allotted_minutes") or 0
                    if not start_at or allotted <= 0:
                        continue
                        
                    start_dt = get_slot_datetime(now_dt, start_at)
                    if not start_dt:
                        continue
                        
                    end_dt = start_dt + timedelta(minutes=allotted)
                    
                    if now_dt >= end_dt:
                        try:
                            await bot.send_message(
                                chat_id=int(student["telegram_id"]),
                                text=f"⏰ *Time Up!* - {task.get('subject')}\n\n"
                                     f"Task: {task.get('topic')}\n\n"
                                     f"Kya aapne ye task complete kar liya? ✅",
                                reply_markup=ReplyKeyboardMarkup([["Yes", "No"]], resize_keyboard=True),
                                parse_mode="Markdown"
                            )
                            update_task(task["id"], {"followup_sent": True})
                            upd_user(int(student["telegram_id"]), {"step": "mentor_manual_completion_check", "current_task_id": str(task["id"])})
                        except Exception as e:
                            logger.error(f"Error sending manual followup: {e}")

                # End of Day Summary for Manual Scheduler
                if manual_tasks and not markers.get(reminder_marker_key("manual_day_summary", date_key)):
                    latest_end = now_dt.replace(hour=0, minute=0, second=0, microsecond=0)
                    for task in manual_tasks:
                        start_at = task.get("scheduled_at")
                        allotted = task.get("allotted_minutes") or 0
                        s_dt = get_slot_datetime(now_dt, start_at)
                        if s_dt:
                            e_dt = s_dt + timedelta(minutes=allotted)
                            if e_dt > latest_end:
                                latest_end = e_dt
                    
                    if now_dt >= latest_end:
                        completed = sum(1 for t in manual_tasks if t.get("status") == "done")
                        total = len(manual_tasks)
                        eff = int((completed / total) * 100) if total > 0 else 0
                        
                        summary_msg = (
                            f"📊 *DAILY MANUAL SUMMARY* ({now_dt.strftime('%d %b')})\n"
                            f"━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"✅ Tasks Completed: {completed}/{total}\n"
                            f"📈 Efficiency: {eff}%\n\n"
                        )
                        if eff >= 80: summary_msg += "Excellent work! Aapne aaj kamaal kar diya! 🌟"
                        elif eff >= 50: summary_msg += "Good effort! Kal aur behtar karte hain. 💪"
                        else: summary_msg += "Thoda aur focus chahiye buddy. Kal phodenge! 🔥"
                        
                        await bot.send_message(chat_id=int(student["telegram_id"]), text=summary_msg, parse_mode="Markdown")
                        
                        if student.get("parent_phone"): # Try to notify parent if possible
                             # Find parent telegram id from users table if available
                             c_db = db(); c_cur = db_cursor(c_db)
                             c_cur.execute("SELECT user_id FROM users WHERE phone LIKE %s", (f"%{student.get('parent_phone')[-10:]}%",))
                             p_row = c_cur.fetchone()
                             c_db.close()
                             if p_row:
                                 await bot.send_message(
                                     chat_id=int(p_row["user_id"]),
                                     text=f"📊 *Daily Study Summary for {student.get('name')}*\n\n"
                                          f"Status: {eff}% Tasks Completed today.\n"
                                          f"Total Tasks: {total}\n"
                                          f"Completed: {completed}",
                                     parse_mode="Markdown"
                                 )
                        
                        markers[reminder_marker_key("manual_day_summary", date_key)] = True
...
async def ask_next_sequential_hw(update, context, student, subjects, index):
    if index >= len(subjects):
        ...
        await context.bot.send_message(chat_id=uid, text=submission_text, parse_mode="Markdown")
    ...
    msg = await context.bot.send_message(
        chat_id=uid,
        text=f"📚 *{index+1}/{total}: {subject}*\n\nAaj {subject} mein kya homework mila? (Ya 'Skip' karo agar nahi mila)",
        reply_markup=ReplyKeyboardMarkup([["Skip"], ["Back", "Ask Doubt"]], resize_keyboard=True),
        parse_mode="Markdown"
    )
    temp["last_wizard_msg_id"] = msg.message_id
...
async def handle_mentorship_message(update: Update, context: ContextTypes.DEFAULT_TYPE, u: Dict[str, Any]) -> bool:
    ...
    if step == "mentor_backlog_ready":
        if text in {"Add Backlog", "Yes"}:
            ...
            # Deleting user msg even for non-wizard steps if we want consistency
            try: await update.message.delete()
            except: pass
    ...
    if step == "mentor_scheduler_options":
        if text == "Add Another Task":
            ...
            # Cleanup wizard msg before finishing
            last_id = temp.get("last_wizard_msg_id")
            if last_id:
                try: await context.bot.delete_message(chat_id=uid, message_id=last_id)
                except: pass
            ...
            # Deleting the user's final response too
            try: await update.message.delete()
            except: pass
    ...
    if step == "mentor_phone":
        ...
        # Cleanup last wizard msg before transition
        temp = get_mentorship_temp(get_user(uid))
        last_id = temp.get("last_wizard_msg_id")
        if last_id:
            try: await context.bot.delete_message(chat_id=uid, message_id=last_id)
    ...
    if step == "mentor_self_study_hours":
        num_match = re.search(r"(\d+)", text)
        if not num_match:
            await wizard_step(update, context, "Hours as number bhejo (e.g., 10).", kb=[["Back", "Ask Doubt"]])
            return True
        val = int(num_match.group(1))
    ...
    if step == "mentor_parent_id":
        ...
        if not (10 <= len(clean_phone) <= 13):
            await wizard_step(update, context, "Valid Parent Telegram Number bhejo (e.g., 9876543210) ya Skip likho.")
            return True
    ...
    if step == "mentor_ready":
        ...
        # If no specific handler caught it, but it matches a dashboard action, let it pass
...
async def handle_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "My Personal Mentor":
        ...
        upd_user(uid, {"step": "mentor_medical_confirm"})
        await wizard_step(update, context, "Kya aap medical leave par jana chahte hain?", kb=YES_NO_OPTIONS)
        return
